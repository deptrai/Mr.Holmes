...
Open Source Intelligence — the practice of gathering information from publicly
available sources on the internet. Mr.Holmes is an OSINT tool focused on
usernames, phone numbers, websites, persons, ports, emails, and dork-based
searches.

## Site List

A JSON file under `Site_lists/` that defines target URLs, error patterns, tags,
and scraping rules per site for a given OSINT subject type (username, phone,
NSFW, etc.).

## Dork

A crafted Google or Yandex search query designed to surface targeted results
for an OSINT investigation. Dork lists live under `Site_lists/Username/` as
`Google_dorks.txt` and `Yandex_dorks.txt`.

## Recap File

A summary artifact (`.txt` or `.mh`) written under `GUI/Reports/...` that
captures findings from an OSINT investigation alongside the JSON-structured
results.

## Staged Profiler

The Autonomous Profiler pipeline (Epic 8/9) that runs a multi-stage OSINT flow:
plugin discovery, parallel data collection, golden-record entity resolution, and
report generation.

## Golden Record

The unified, deduplicated entity record produced by `EntityResolver` from
multiple plugin data sources, using Jaro-Winkler string similarity and
perceptual hashing for merge decisions.

## Plugin

A pluggable OSINT data source in the Autonomous Profiler. Plugins register with
`PluginManager`, share an `aiohttp.ClientSession` via `get_http_session()`, and
cache results through `PluginCache` (SQLite).

## Proxy Identity

The anonymization layer managed by `Core.Support.Proxies` that routes HTTP
requests through proxies and resolves proxy geo-identity via `ip-api.com`.
