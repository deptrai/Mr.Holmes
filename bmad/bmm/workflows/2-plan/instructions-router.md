...
c. Working with messy/legacy code (brownfield - needs analysis)

**3. What are you building?** (brief description)
</ask>

<action>Capture project_type and project_context</action>
<action>Detect if project_type == "game"</action>

<check if='project_context == "c" OR project_context contains "brownfield" OR project_context contains "legacy"'>
  <action>Set is_brownfield = true</action>
  ...
  <check if="brownfield_analysis_not_found">
    <output>⚠️ **BROWNFIELD PROJECT DETECTED - ANALYSIS REQUIRED**
...
Would you like to:
1. Run brownfield-analysis workflow now (RECOMMENDED)
2. Continue anyway (NOT RECOMMENDED - may miss critical constraints)
3. Exit and run analysis manually

Please select an option:</output>

    <ask>What would you like to do?</ask>

    <check if="user_selects_1">
      <output>✅ Excellent choice! Launching brownfield-analysis workflow...</output>
      <invoke-workflow>{project-root}/bmad/bmm/workflows/1-analysis/brownfield-analysis/workflow.yaml</invoke-workflow>
      <action>After brownfield analysis completes, resume plan-project workflow</action>
      <action>Load brownfield analysis findings</action>
      <action>Store as brownfield_context for use in planning</action>
    </check>

    <check if="user_selects_2">
      <output>⚠️ **WARNING:** Proceeding without brownfield analysis.

**Risks:**
- May miss critical architectural constraints
- Could introduce conflicting patterns
- Technical debt may compound
- Integration issues likely
- Refactoring costs higher

**Recommendation:** Run brownfield-analysis after this planning session.

Continuing with planning workflow...</output>
      <action>Set brownfield_warning = true</action>
      <action>Add warning to PRD/analysis output</action>
    </check>

    <check if="user_selects_3">
      <output>✅ Good decision! Please run:
...
Exiting plan-project workflow...</output>
      <action>Exit workflow gracefully</action>
    </check>
  </check>

  <check if="brownfield_analysis_found">
    ...
    <action>Store as brownfield_context</action>
    <output>
**Brownfield Context Loaded:**
- Architecture: {{architecture_overview_summary}}
- Critical Debt: {{critical_debt_count}} items
- Security Risk: {{security_risk_level}}
- Test Coverage: {{test_coverage}}

This context will inform your planning decisions.
    </output>
  </check>
</check>

<check if='project_context == "a" OR project_context contains "greenfield" OR project_context contains "new"'>
  <output>✅ Greenfield project detected. Proceeding with standard planning.</output>
  <action>Set is_brownfield = false</action>
</check>
